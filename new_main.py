...
async def declare_and_bind_logging(channel: aio_pika.Channel):
    ...
    print(f"Logging Queue bound to exchange {EXCHANGE_NAME} with routing key {LOGGING_ROUTING_KEYS}")
    return queue
        ...
        async for message in queue_iter:
            async with message.process():
                try:
                    ...
                    latest_messages[device_id] = new_data  # Store only the latest message
...
async def save_csv_file(keys, data, device_id):
    ...
    target_file = documents_path / f"log_{device_id}_{current_hour.strftime('%Y-%m-%d_%H')}.csv"
    ...
    while True:
        try:
            ...
            async with connection:
                ...
                redis_consumer_task = asyncio.create_task(consume_and_store_redis(channel))
